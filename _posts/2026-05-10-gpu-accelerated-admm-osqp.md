...
<tr><td class="indent">1.</td><td>$(x^{k+1}, z^{k+1}) \leftarrow \arg\min_{x,z:Ax=z} \frac{1}{2}x^T P x + q^T x + \frac{\sigma}{2}\|x - x^k\|_2^2 + \frac{1}{2}\|z - z^k + R^{-1}y^k\|_2^2$</td></tr>
...
<tr><td class="indent">4.</td><td>$y^{k+1} \leftarrow y^k + R(z^{k+1} - \alpha \tilde{z}^{k+1} - (1-\alpha)z^k)$</td></tr>
...
$$(P + \sigma I + \bar{\rho} A^T A)\tilde{x}^{k+1} = \sigma x^k - q + A^T(\bar{\rho}z^k - y^k) \tag{14}$$
