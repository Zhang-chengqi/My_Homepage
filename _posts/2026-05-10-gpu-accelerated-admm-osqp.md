...
<tr><td colspan="2"><strong>given</strong> initial values $x^0, z^0, y^0$ and parameters $\sigma > 0$, $R \in \mathbb{S}_+^n$, $\alpha \in [0,2]$</td></tr>
<tr><td colspan="2">Set $k = 0$</td></tr>
<tr><td colspan="2"><strong>repeat</strong></td></tr>
<tr><td class="indent">1.</td><td>$(\tilde{x}^{k+1}, \tilde{z}^{k+1}) \leftarrow \arg\min_{\tilde{x},\tilde{z}:A\tilde{x}=\tilde{z}} \frac{1}{2}\tilde{x}^T P \tilde{x} + q^T \tilde{x} + \frac{\sigma}{2}\|\tilde{x} - x^k\|_2^2 + \frac{1}{2}\|\tilde{z} - z^k + R^{-1}y^k\|_2^2$</td></tr>
<tr><td class="indent">2.</td><td>$x^{k+1} \leftarrow \alpha \tilde{x}^{k+1} + (1-\alpha)x^k$</td></tr>
<tr><td class="indent">3.</td><td>$z^{k+1} \leftarrow \Pi_{[l,u]}(\alpha \tilde{z}^{k+1} + (1-\alpha)z^k - R^{-1}y^k)$</td></tr>
<tr><td class="indent">4.</td><td>$y^{k+1} \leftarrow y^k + R(\alpha \tilde{z}^{k+1} + (1-\alpha)z^k - z^{k+1})$</td></tr>
<tr><td class="indent">5.</td><td>$k \leftarrow k + 1$</td></tr>
<tr><td colspan="2"><strong>until</strong> termination criterion is satisfied</td></tr>
</table>
...
Px + q + A^T y &= 0 \tag{3b} \\
l \leq z \leq u \tag{3c} \\
y_i = 0, \quad \text{if } z_i = l_i \text{ or } z_i = u_i \tag{3d}
\end{align}$$
...
$$\begin{bmatrix} P + \sigma I & A^T \\ A & -R^{-1} \end{bmatrix} \begin{bmatrix} \tilde{x}^{k+1} \\ \nu^{k+1} \end{bmatrix} = \begin{bmatrix} \sigma x^k - q \\ z^k -R^{-1}y^k \end{bmatrix} \tag{7}$$

我们称(7)中的矩阵为KKT矩阵。OSQP使用直接方法，通过首先计算KKT矩阵的$LDL^T$分解来计算精确解。由于KKT矩阵不依赖于迭代计数器 $k$，OSQP在算法开始时执行分解，并在后续迭代中重用这些因子。
...
<table class="algorithm">
<tr><td colspan="2"><strong>Require:</strong> initial value $x^0$, preconditioner $M$</td></tr>
<tr><td class="indent">1.</td><td>$r^0 \leftarrow Kx^0 - b, y^0 \leftarrow M^{-1}r^0, p^0 \leftarrow -y^0$</td></tr>
<tr><td class="indent">2.</td><td>Set $k = 0$</td></tr>
<tr><td colspan="2"><strong>while</strong> $\|r^k\| > \varepsilon \|b\|$ <strong>do</strong></td></tr>
<tr><td class="indent">3.</td><td>$\alpha^k \leftarrow -\frac{(r^k)^T y^k}{(p^k)^T K p^k}$</td></tr>
<tr><td class="indent">4.</td><td>$x^{k+1} \leftarrow x^k + \alpha^k p^k$</td></tr>
<tr><td class="indent">5.</td><td>$r^{k+1} \leftarrow r^k + \alpha^k K p^k$</td></tr>
<tr><td class="indent">6.</td><td>$y^{k+1} \leftarrow M^{-1} r^{k+1}$</td></tr>
<tr><td class="indent">7.</td><td>$\beta^{k+1} \leftarrow \frac{(r^{k+1})^T y^{k+1}}{(r^k)^T y^k}$</td></tr>
<tr><td class="indent">8.</td><td>$p^{k+1} \leftarrow -y^{k+1} + \beta^{k+1} p^k$</td></tr>
<tr><td class="indent">9.</td><td>$k \leftarrow k + 1$</td></tr>
<tr><td colspan="2"><strong>end while</strong></td></tr>
</table>
...
$$(P + \sigma I + A^T R A)\tilde{x}^{k+1} = \sigma x^k - q + A^T(R z^k - y^k) \tag{14}$$

注意上述系数矩阵不必显式形成。矩阵-向量积：

$$r \leftarrow (P + \sigma I + \bar{\rho} A^T A)x$$

可以通过以下方式计算：

$$\begin{aligned}
z &\leftarrow \bar{\rho} A x \\
...
$$\text{diag}(M) = \text{diag}(P) + \sigma \mathbf{1} + \bar{\rho} \cdot \text{diag}(A^T A)$$
...
本文探索了利用GPU的大规模并行化能力来加速大规模QP的求解。cuOSQP的实现建立在OSQP基础上，这是一个基于ADMM的最先进QP求解器。显著加速是通过使用PCG方法求解ADMM中的线性系统以及并行化所有向量和矩阵运算实现的。
